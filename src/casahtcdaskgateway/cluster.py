...
class HTCGatewayCluster(GatewayCluster):
    ...
    def __init__(self, image_registry="registry.hub.docker.com", apptainer_image='coffeateam/coffea-base-almalinux8:0.7.22-py3.10', node_match_expr='rank = (isDaskNode == True)',
                 **kwargs):
        self.scheduler_proxy_ip = kwargs.pop('', '129.93.179.11')
    ...
    def scale_batch_workers(self, n):
        ...
        tmproot = f"/home/cms-jovyan/tmp/{cluster_name}"
...
export APPTAINERENV_DASK_GATEWAY_API_URL="https://dask-gateway-api/api"
...
dask worker --name $2 --tls-ca-file /etc/dask-credentials/dask.crt --tls-cert /etc/dask-credentials/dask.crt --tls-key /etc/dask-credentials/dask.pem --worker-port 10000:10070 --no-nanny --local-directory /srv --scheduler-sni daskgateway-"""+cluster_name+""" --nthreads 1 tls://"""+self.scheduler_proxy_ip+""":80"""
        ...
        cmd = "/usr/local/bin/condor_submit -spool htcdask_submitfile.jdl | grep -oP '(?<=cluster )[^ ]*'"
